...
try:
    ...
    print("failed to set default workspace path, if the program is run in exe file PLEASE IGNORE this message.")

with open("./config/setting.json","r+") as f:
    setting = json.load(f)
time_interval = int(setting["AutoGALTimeInterval"])

with open("./config/api-config.json","r+") as f:
    api_config = json.load(f)
ocr_config = api_config['ocr_config']
translate_config = api_config['translate_config']

def GetLatestVersion(): # 用于检查软件版本是否是最新版本
    try:
...
class MultiProcessGetResultWithoutArgs(threading.Thread): # 用于在后台线程中执行一些计算或操作，避免阻塞主线程。
...
def on_activate_OCR(): # 用于激活OCR功能，通过热键触发。
    global HotKeyEvent
    HotKeyEvent = True
    if not BanHotKey.get():
        onlyOCR()

def on_activate_GAL(): # 用于激活GAL（Galgame）游戏模式，通过热键触发。
    global HotKeyEvent
    HotKeyEvent = True
    if not BanHotKey.get():
        galgameMode()

def showTip(Text): # 用于在桌面上弹出提示消息。
...
def browserTxAPI(): # 用于快速访问腾讯云api相关的控制台页面，方便用户进行API密钥配置等操作。
    open_new_tab("https://console.cloud.tencent.com/cam/capi")

def browserBdAPI(): # 用于快速访问百度AI OCR控制台页面，便于用户进行OCR相关设置和API管理。
...
#         'cookie': Cookie
#     }
#     with open("config/baidutrans.js", 'r') as f:
...
def clearClipboard(): # 用于清空系统剪贴板的内容，通常在不需要保留剪贴板数据时使用。
    win32clipboard.OpenClipboard()
    win32clipboard.EmptyClipboard()
    win32clipboard.CloseClipboard()

def oneShot(): # 模拟按下 Ctrl + Alt + A 的组合键。
    with Controll.pressed(Key.ctrl):
        with Controll.pressed(Key.alt):
            Controll.press('a')
            Controll.release('a')

def GetPointer(): # 获取鼠标点击和释放的位置，并在按下 Esc 键时停止监听。
...
def getNowOCR(): # 获取当前默认的OCR选项。
    nowOCR = setting['defaultOCR']
    return OcrPri.index(nowOCR)

def getNowLang(): # 设置当前OCR语言和Google翻译语言。
...
def saveSetting(*args): # 用于保存用户设置，并根据设置决定是否将程序添加为开机启动项。
    ...
    for j in AllSetting: #遍历所有下拉框设置内容
        ...
        SettingList.append({**OcrAll, **LanguageAll, **GalAll}[eval(j + ".get()")])  # 获取选项内容转换Json内容
...
def changeOCR(*args): # 根据OCR设置的选项调整OCR语言设置的可选项。
...
def openSettingFile(*args): # 用于打开 setting.json 配置文件，调用系统命令打开记事本（notepad）并显示该文件。
    os.system("start /b notepad ./config/setting.json")

def returnToPrimary(): # 用于重置或获取当前的一些配置选项，包括OCR、语言设置、翻译设置以及其他设置。
    getNowOCR()
    getNowLang()
    getNowTrans()
    getOtherSetting()

def feedBack(): # 用于打开GitHub上的反馈页面并提示用户提交问题。
    open_new_tab("https://github.com/jizhihaoSAMA/OCR-Translation-SystemTray/issues")
    messagebox.showinfo(u"提示",u"提交issue就可以辣。（¯﹃¯）")

def checkWhetherGet(win,thread,position): # 用于检查线程的结果并将结果显示在窗口中。如果线程没有结果，它会继续检查，直到获得结果。
...
def OCR_Core(Image,**kwargs):
    ...
    if kwargs.get("EXTRA"):
        ...
        #写入内存，太慢，更换保存本地方式
        Image.save('config/LastImage.jpg')
        with open('config/LastImage.jpg', 'rb+') as f:
            LastImageValue = f.read()
    OCRText = ""
    if SelectOCR == "bd_normal" or SelectOCR =="bd_accurate":
        AppID = ocr_config["bd_api"]["AppID"]
        APIKey = ocr_config["bd_api"]["APIKey"]
        SecretKey = ocr_config["bd_api"]["SecretKey"]
...
def Text2Novel(): # 将OCR识别结果转化为小说格式的文本
...
def ComeToPrimaryFormat(): # 将文本恢复到初始格式
...
def OCR(): # OCR主函数
    ...
    oneShot()
    GetPointer()
    print(f'Click_x, Click_y, Release_x, Release_y: {(Click_x, Click_y, Release_x, Release_y)}')
    image = ImageGrab.grab((Click_x, Click_y, Release_x+1, Release_y+1)) #一个像素差
...
def changeOCRAndOCR(*args): # 更改OCR引擎并更新OCR结果文本框。
    global OCRText
    OCRText = OCR_Core(None,EXTRA=True) #可优化
    OCRResultText.delete(1.0, "end")
    OCRResultText.insert("end", OCRText)

def continueOCR(): # 继续进行OCR识别，处理鼠标点击事件并更新OCR结果。
    OCRWin.withdraw()
    oneShot()
    GetPointer()
    while True:
...
def onlyOCR(): # 执行OCR识别并显示结果，处理OCR窗口显示、层级设置和显示文本等。
    ...
    try:
        ...
        OCRWin.geometry('+{}+{}'.format(Win.winfo_geometry().split("+")[1],Win.winfo_geometry().split("+")[2]))
...
def autoGAL():
    if Auto.get() == 1:
        ...
        NowImage.save('config/NowImage.jpg')
        with open('config/NowImage.jpg','rb+') as f:
        ...
        if NowImageValue != LastImageValue:  # 区域不一样，继续OCR
            ...
            GALResultText.insert("end", tx_translate(OCRText, translate_config))
        GALResultText.after(time_interval,autoGAL)
    else:
...
def galgameMode():#galgame游戏吞按键
    try:
        ...
        GALWin.geometry('+{}+{}'.format(Win.winfo_geometry().split("+")[1],Win.winfo_geometry().split("+")[2]))
...
try:
    ...
    Win.bind_all("<Control-o>",lambda event: os.system("start /b notepad ./config/setting.json"))
